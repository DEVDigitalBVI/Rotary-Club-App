...
export async function getCompletedOnboarding(memberId: string): Promise<OnboardingKey[]> {
  ...
  const [recorded, member, committees, rsvps, legacyProjects, projects] = await Promise.all([
    ...
    supabase.from("event_rsvps").select("event_id").eq("member_id", memberId).eq("status", "yes").eq("registration_status", "registered").limit(1),
    supabase.from("project_volunteers").select("project_id").eq("member_id", memberId).limit(1),
    supabase.from("project_slot_signups").select("slot_id, project_slots!inner(cancelled)").eq("member_id", memberId).eq("status", "registered").eq("project_slots.cancelled", false).limit(1),
  ]);
  [recorded.error, member.error, committees.error, rsvps.error, projects.error, legacyProjects.error].forEach((error) => throwOnSupabaseError(error, "Unable to load onboarding progress"));

  const completed = new Set<OnboardingKey>((recorded.data ?? []).filter(row => row.task_key === "directory").map((row) => row.task_key));
  ...
  if ((projects.data?.length ?? 0) > 0 || (legacyProjects.data?.length ?? 0) > 0) completed.add("first-project");
  return [...completed];
}
