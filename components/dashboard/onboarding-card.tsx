...
import { setOnboardingTaskAction } from "@/app/(app)/onboarding/actions";
    ...
    <section className="mx-4 mt-5 overflow-hidden rounded-[1.5rem] border border-border bg-card sm:mx-8 lg:mx-10">
      <div className="grid lg:grid-cols-[.7fr_1.3fr]">
        ...
        <div className="divide-y divide-border px-6">
          {onboardingTasks.map((task) => { const done = completed.includes(task.key); return <form key={task.key} action={setOnboardingTaskAction.bind(null, task.key, !done)}><button type="submit" className="group flex w-full items-start gap-3 py-4 text-left"><span className={`mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-full ${done ? "bg-primary text-primary-foreground" : "border border-border text-muted-foreground"}`}>{done ? <Check className="size-3.5" /> : <Circle className="size-2" />}</span><span><strong className={`block text-sm ${done ? "text-muted-foreground line-through" : "text-foreground"}`}>{task.title}</strong><span className="mt-0.5 block text-xs leading-5 text-muted-foreground">{task.detail}</span></span></button></form>; })}
        </div>
      </div>
    </section>
  );
