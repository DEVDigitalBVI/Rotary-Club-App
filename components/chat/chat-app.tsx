...
import { ArrowLeft, CalendarDays, ChevronDown, Hash, HeartHandshake, Loader2, MessageCirclePlus, MoreHorizontal, Reply, Search, Send, ShieldCheck, SmilePlus, Trash2, Users, X } from "lucide-react";
...
import { deleteChatMessageAction, markChatReadAction, sendChatMessageAction, startDirectChatAction, toggleChatReactionAction } from "@/app/(app)/chat/actions";
...
export function ChatApp({ channels, members, currentMemberId, canModerate, initialChannelId }: { channels: ChatChannel[]; members: Member[]; currentMemberId: string; canModerate: boolean; initialChannelId?: string }) {
  ...
  useEffect(() => { threadEndRef.current?.scrollIntoView({ block: "end" }); }, [selected?.messages.length]);
  ...
  return <div className="grid h-[calc(100dvh-10.5rem)] min-h-[540px] overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[var(--shadow-card)] md:grid-cols-[19rem_minmax(0,1fr)]">
    <aside className={cn("min-w-0 flex-col border-r border-border bg-muted/25", mobileShowThread ? "hidden md:flex" : "flex")}>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-3">
        ...
        <ChannelSection label="My groups" channels={data.filter((channel) => ["committee", "event", "project"].includes(channel.kind))} {...{ selectedId, currentMemberId, memberById, displayName }} onSelect={selectChannel} />
        <ChannelSection label="Direct messages" channels={data.filter((channel) => channel.kind === "dm")} {...{ selectedId, currentMemberId, memberById, displayName }} onSelect={selectChannel} />
      </div>
    </aside>
    <section className={cn("min-w-0 flex-col", mobileShowThread ? "flex" : "hidden md:flex")}>
      {selected ? <>
        <header className="flex min-h-16 items-center gap-3 border-b border-border px-3 sm:px-5"><Button variant="ghost" size="icon-sm" className="md:hidden" onClick={() => setMobileShowThread(false)} aria-label="Back to conversations"><ArrowLeft /></Button><ChannelIcon kind={selected.kind} /><div className="min-w-0 flex-1"><h2 className="truncate text-sm font-semibold text-foreground">{displayName(selected)}</h2><p className="truncate text-xs text-muted-foreground">{channelDescription(selected.kind)}</p></div><div className="relative hidden w-56 sm:block"><Search className="absolute left-3 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search conversation" className="h-9 pl-8 text-xs" /></div></header>
        <div className="border-b border-border p-3 sm:hidden"><div className="relative"><Search className="absolute left-3 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search this conversation" className="h-9 pl-8 text-xs" /></div></div>
        <div className="flex-1 overflow-y-auto px-3 py-5 sm:px-6">{visibleMessages.map((message, index) => <MessageRow key={message.id} message={message} previous={visibleMessages[index - 1]} messages={selected.messages} memberById={memberById} currentMemberId={currentMemberId} canModerate={canModerate} onReply={setReplyToId} onError={setError} />)}{visibleMessages.length === 0 && <EmptyConversation searching={Boolean(query)} name={displayName(selected)} />}<div ref={threadEndRef} /></div>
...
function ChannelSection({ label, channels, selectedId, currentMemberId, memberById, displayName, onSelect }: { label: string; channels: ChatChannel[]; selectedId: string; currentMemberId: string; memberById: Map<string, Member>; displayName: (channel: ChatChannel) => string; onSelect: (id: string) => void }) {
  ...
  return <div className="mb-5"><p className="mb-1 px-3 text-[11px] font-semibold uppercase tracking-[.14em] text-muted-foreground">{label}</p>{channels.map((channel) => {
    ...
    return <button key={channel.id} onClick={() => onSelect(channel.id)} className={cn("group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition-colors hover:bg-muted", channel.id === selectedId && "bg-primary/10 text-primary hover:bg-primary/10")}>{channel.kind === "dm" ? <MemberAvatar member={memberById.get(otherId ?? "")} className="size-9 shrink-0" fallbackClassName="text-xs" /> : <div className={cn("flex size-9 shrink-0 items-center justify-center rounded-lg bg-background text-muted-foreground shadow-sm", channel.id === selectedId && "bg-primary text-primary-foreground")}><ChannelIcon kind={channel.kind} /></div>}<div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className={cn("truncate text-sm", unread ? "font-semibold text-foreground" : "font-medium")}>{displayName(channel)}</p>{unread > 0 && <span className="ml-auto flex min-w-5 items-center justify-center rounded-full bg-primary px-1.5 text-[10px] font-bold text-primary-foreground">{Math.min(unread, 99)}</span>}</div><p className={cn("mt-0.5 truncate text-xs text-muted-foreground", unread && "text-foreground/70")}>{last ? `${memberById.get(last.senderId)?.name.split(" ")[0] ?? "Member"}: ${last.body}` : channel.kind === "dm" ? "Start a conversation" : channelDescription(channel.kind)}</p></div></button>;
  })}</div>;
}

function MessageRow({ message, previous, messages, memberById, currentMemberId, canModerate, onReply, onError }: { message: ChatMessage; previous?: ChatMessage; messages: ChatMessage[]; memberById: Map<string, Member>; currentMemberId: string; canModerate: boolean; onReply: (id: string) => void; onError: (error: string) => void }) {
...
function NewDirectMessage({ members, currentMemberId, onCreated }: { members: Member[]; currentMemberId: string; onCreated: (id: string) => void }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger render={<Button variant="outline" size="icon-sm" aria-label="New direct message" />}><MessageCirclePlus /></DialogTrigger><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>New conversation</DialogTitle></DialogHeader><div className="relative"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find a club member" className="pl-9" autoFocus /></div><div className="max-h-80 overflow-y-auto">{choices.map((member) => <button key={member.id} disabled={pending} onClick={() => startTransition(async () => { const id = await startDirectChatAction(member.id); setOpen(false); onCreated(id); })} className="flex w-full items-center gap-3 rounded-xl p-3 text-left hover:bg-muted"><MemberAvatar member={member} className="size-10" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{member.name}</p><p className="truncate text-xs text-muted-foreground">{member.classification}</p></div>{pending ? <Loader2 className="size-4 animate-spin" /> : <ChevronDown className="size-4 -rotate-90 text-muted-foreground" />}</button>)}</div></DialogContent></Dialog>;
}
...
function channelDescription(kind: ChatChannel["kind"]) { return { club: "Open to all active club members", committee: "Private committee room", event: "For registered attendees", project: "For project volunteers", dm: "Private conversation" }[kind]; }
