...
export function FeedbackForm({ready}:{ready:boolean}){
 const { attach: restoreDraft, onInput: saveDraft, clear: clearDraft }=useFormDraft("feedback");
 ...
 <form ref={restoreDraft} onInput={saveDraft} className="mt-6 space-y-4" onSubmit={event=>{event.preventDefault();const form=event.currentTarget;const data=new FormData(form);request.current??=crypto.randomUUID();setResult({});start(async()=>{try{const saved=await submitFeedback(request.current!,data);setResult(saved);if(saved.success){clearDraft();form.reset();request.current=null;}}catch{setResult({error:"Couldn’t confirm submission. Retry to check without sending a duplicate."});}});}}>
