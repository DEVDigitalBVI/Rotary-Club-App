...
async function StayConnected() {
  ...
  return <section className="mx-4 mt-5 rounded-2xl border border-border bg-card p-5 sm:mx-8 lg:mx-10"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3"><h2 className="text-xl">Stay connected</h2><Link href="/directory" className="text-sm font-semibold text-primary">Meet your club members →</Link></div>{messages === null ? <p className="mt-3 text-sm text-muted-foreground">Conversation preview is temporarily unavailable.</p> : messages.slice(0,1).map(message => <div key={message.id} className="my-3"><p className="text-sm font-semibold">{message.channel}</p><p className="line-clamp-2 text-sm text-muted-foreground">{message.body}</p></div>)}<div className="mt-3 flex flex-wrap gap-x-5 gap-y-2"><Link href="/chat" className="text-sm font-semibold text-primary"><MessageCircle className="mr-1 inline size-4"/>Open conversations</Link><Link href="/feedback" className="text-sm font-semibold text-primary">Share an idea →</Link></div></section>;
}
