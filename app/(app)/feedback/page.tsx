...
export default async function FeedbackPage({searchParams}:{searchParams:Promise<{view?:string;page?:string;status?:string;category?:string;unanswered?:string}>}){
 ...
 <form className="mb-5 flex flex-wrap items-end gap-3 rounded-xl border border-border p-4">
 {review&&<input type="hidden" name="view" value="review"/>}
 <label className="text-sm">Status<select name="status" defaultValue={params.status??""} className="ml-2 rounded border bg-background p-2"><option value="">All</option>{Object.entries(feedbackStatuses).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label>
 <label className="text-sm">Category<select name="category" defaultValue={params.category??""} className="ml-2 rounded border bg-background p-2"><option value="">All</option>{Object.entries(feedbackCategories).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label>
 <label className="text-sm"><input type="checkbox" name="unanswered" value="1" defaultChecked={params.unanswered==="1"}/> Unanswered only</label><button className="rounded bg-primary px-3 py-2 text-sm text-primary-foreground">Apply filters</button>
 </form>
